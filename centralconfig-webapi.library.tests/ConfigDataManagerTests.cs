...
namespace centralconfig_webapi.library.tests
{
    ...
    public class ConfigDataManagerTests
    {
        ...
        /// Our returned data
        /// </summary>
        private IQueryable<configitem> data = null;
        ...
        public void Test_Setup()
        {
            ...
            mockSet.As<IQueryable<configitem>>().Setup(m => m.Provider).Returns(data.Provider);
            mockSet.As<IQueryable<configitem>>().Setup(m => m.Expression).Returns(data.Expression);
            mockSet.As<IQueryable<configitem>>().Setup(m => m.ElementType).Returns(data.ElementType);
            mockSet.As<IQueryable<configitem>>().Setup(m => m.GetEnumerator()).Returns(data.GetEnumerator());
        ...
        public void Remove_ValidConfigItem_Successful()
        {
            //  Arrange
            ConfigDataManager manager = new ConfigDataManager(mockContext.Object);
            List<ConfigItem> itemsBefore = manager.GetAll();    //  Get the list prior (as a baseline)

            //  Act            
            manager.Remove(new ConfigItem { Application = "SomeOtherApp", Name = "SpecificConfig1" });
            List<ConfigItem> itemsAfter = manager.GetAll(); //  Get the list again to check

            //  Assert
            // Assert.IsTrue(itemsBefore.Count > itemsAfter.Count);
        }
        ...
        public void Set_NewConfigItem_SuccessfullyAdds()
        {
            ...
            // Assert.AreNotEqual(0, retval.Id);
        }
    }
}
